...
const convertToContentTagText = (tag: string) => {
  ...
  if (tag == "red") {
    title = "Gender";
    desc =
      "can range from: being the gender you are, gender discrimination, sexual harassment";
  } else if (tag == "orange") {
    title = "Race";
    ...
    desc =
      "disabled people, immigrants, basically people who are not young 20s-30s American citizens from privileged backgrounds who graduated from university";
  } else if (tag == "green") {
    title = "Interpersonal drama";
    desc =
      "can range from: from feeling misunderstood at work to psychological manipulation";
  } else {
    ...
    return (
      <div
        className={clsx(
          cardStyles,
          props.styleName,
          convertToContentTagClass(contentTag),
        )}
